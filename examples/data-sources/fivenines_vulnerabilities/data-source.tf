# Every patchable Critical finding in the organization.
...
resource "terraform_data" "web_gate" {
  ...
  lifecycle {
    ...
    # below would fail the plan on it too - this precondition fails with a
...
data "fivenines_vulnerabilities" "base_image" {
  docker_image_id = data.fivenines_docker_images.nginx.images[0].id
}
