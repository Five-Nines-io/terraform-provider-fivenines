...
func TestClient_ListHostGroups_Pagination(t *testing.T) {
	...
	_, c := newTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if n == 1 {
			json.NewEncoder(w).Encode(map[string]interface{}{
				"host_groups": []map[string]interface{}{{"id": 1, "name": "A", "position": 1}},
				"meta":        map[string]int{"count": 1, "total": 2, "offset": 0},
			})
			return
		}
		json.NewEncoder(w).Encode(map[string]interface{}{
			"host_groups": []map[string]interface{}{{"id": 2, "name": "B", "position": 2}},
			"meta":        map[string]int{"count": 1, "total": 2, "offset": 1},
		})
	})
	...
	if groups[1].Name != "B" {
		t.Errorf("expected second group B, got %s", groups[1].Name)
	}
}
