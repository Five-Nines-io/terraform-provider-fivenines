	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"docker_image_id": schema.StringAttribute{
				Description: "Scope the findings to one container image (the `id` from `fivenines_organization_docker_images`, not the digest). " +
			...
			"order": schema.StringAttribute{
				Description: "Sort column. Defaults to `cvss_score` — the first page of a security list has to be the worst rows, not the oldest. Unscored findings sort LAST in both directions rather than heading a descending list.",
				Optional:    true,
				Validators: []validator.String{
					stringvalidator.OneOf("cvss_score", "package_name", "created_at", "updated_at"),
				},
			},
...
func (d *vulnerabilitiesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := client.VulnerabilityListOptions{
		Patchable:       filterBool(state.Patchable),
		FixState:        filterString(state.FixState),
		PackageName:     filterString(state.PackageName),
		VulnerabilityID: filterString(state.VulnerabilityID),
	...
	if !state.Severity.IsNull() && !state.Severity.IsUnknown() {
		resp.Diagnostics.Append(state.Severity.ElementsAs(ctx, &opts.Severity, false)...)
		if resp.Diagnostics.HasError() {
			return
		}
	}
	...
	case !state.InstanceID.IsNull():
		list, err = d.client.ListInstanceVulnerabilities(ctx, state.InstanceID.ValueString(), opts)
	case !state.DockerImageID.IsNull():
		list, err = d.client.ListDockerImageVulnerabilities(ctx, state.DockerImageID.ValueString(), opts)
	default:
		list, err = d.client.ListVulnerabilities(ctx, opts)
	}
	if err != nil {
